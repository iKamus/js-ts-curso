...
Node te da acceso al disco duro de la compu y a informacion del sistema donde corre tu programa. Es como si tuvieras las llaves de la conserjeria: puedes leer archivos, escribirlos, borrarlos, y ademas saber quien es, donde esta y que le pasaron al programa cuando lo arrancaste.
...
- **Sintaxis**:
```js
console.log(__dirname);   // D:\proyecto\13-node-archivos\ejercicios
console.log(__filename);  // D:\proyecto\13-node-archivos\ejercicios\ej-01-lineas.js
```
- **Errores comunes**:
...
- **Que es**: un modulo de Node para trabajar con rutas de archivos. Se encarga de usar el separador correcto segun el sistema operativo (`\` en Windows, `/` en Linux/Mac). Es como un GPS que arma la direccion por ti sin que te importen los detalles del camino.
...
- **Errores comunes**:
  ...
  - Usar `resolve` cuando solo necesitas juntar fragmentos: `resolve` siempre devuelve una absoluta, lo cual puede no ser lo que quieres.
...
```
- **Errores comunes**:
  - No manejar errores: si el archivo no existe, `readFileSync` lanza una excepcion. Envuelve en try/catch o usa `existsSync` antes.
  - Olvidar `'utf8'`: sin el encoding, `readFileSync` devuelve un Buffer, no un string.
  - `writeFileSync` sobreescribe todo el archivo. Si quieres agregar, usa `appendFileSync`.
- **Buenas practicas**: usa `existsSync` antes de leer o borrar para evitar crashes. Para archivos grandes, prefiere la version asincrona. Siempre construye rutas con `path.join`.
...
- **Buenas practicas**: siempre usa `async/await` con fs/promises. Envuelve en try/catch para manejar errores. Es el estandar moderno en Node.
...
```
- **Errores comunes**:
  - `JSON.parse` con texto invalido lanza `SyntaxError`. Siempre envuelve en try/catch.
...
El modulo usa archivos de prueba en la carpeta `data`:
...
| Archivo | Consigna |
|---|---|
| `proyecto/organizador.js` | Organizador de archivos: leer un directorio, filtrar por extension, mover/copiar archivos, generar reporte JSON |
...
Ejemplo de fs.promises con async/await:
```js
const fs = require('fs/promises');
const path = require('path');
...
leerArchivo().catch(console.error);
```

Para probar los ejercicios, ejecuta cada uno con Node:
```
node ejercicios/ej-01-lineas.js
node ejercicios/ej-02-registro.js
node ejercicios/ej-03-palabras.js
node ejercicios/ej-04-tareas.js
node ejercicios/ej-05-renombrar.js
node proyecto/organizador.js
```
