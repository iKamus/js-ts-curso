...
- **Cuando usarla**: cuando quieres que tu codigo sea predecible y facil de probar. Si una funcion pura falla, el problema esta en los datos que le llegaron, no en algun lado escondido.
- **Sintaxis**:
```js
...
- **Buenas practicas**:
  - Si necesitas "guardar" algo, devuelve un valor nuevo, no mutes el original.
...
- **Que es**: no modificar datos existentes, sino crear copias nuevas con los cambios. Es como fotocopiar un documento antes de escribirle: el original queda intacto y la copia tiene tus anotaciones.
...
- **Buenas practicas**:
  ...
  - Nombra las funciones callback si son complejas: mejora la legibilidad.
...
- **Que es**: transformar una funcion que recibe varios argumentos de golpe `f(a, b)` en una cadena de funciones que reciben UN argumento cada una `f(a)(b)`. Es como armar un sandwich por etapas: primero eliges el pan, despues el relleno, y asi sucesivamente.
...
- **Buenas practicas**:
  - Nombra las versiones curriadas especializadas: `const iva21 = sumarIVA(0.21)`.
...
- **Cuando usarla**: cuando necesitas transformar datos en varios pasos y quieres que el codigo sea legible y reutilizable.
- **Sintaxis**:
```js
...
- **Buenas practicas**:
  - Empieza con composicion manual y despues puedes explorar utilities como `pipe`.
  - Mantén las funciones chicas y con una sola responsabilidad.
...
| Archivo | Consigna |
|---|---|
| `proyecto/pipeline.js` | Fabrica de transformaciones: crear funciones puras, aplicar composicion para transformar datos paso a paso |

---

Ejemplo de composicion pipeline:
```js
const toUpper = s => s.toUpperCase();
const addExclamation = s => s + '!';
const addInfo = s => `[${s}]`;

// Composicion en orden (de derecha a izquierda)
const pipeline = [toUpper, addExclamation, addInfo];
const resultado = pipeline.reduce((s, fn) => fn(s), 'hola');
...
```
node ejercicios/ej-01-contador.js
node ejercicios/ej-02-multiplicador.js
node ejercicios/ej-03-descuentos.js
node ejercicios/ej-04-once.js
node proyecto/pipeline.js
```
