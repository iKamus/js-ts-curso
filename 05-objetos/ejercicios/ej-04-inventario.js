...
Mirá, tenés el stock del almacén (un objeto donde cada casilla es un
producto y su cantidad):
  let inventario = { manzanas: 10, peras: 5, naranjas: 0 };

Acá vas a crear funciones que trabajen con el objeto. Para leer o
cambiar una casilla cuyo nombre viene "en una variable", se usa
...
1) agregar(nombre, cantidad) → le suma cantidad al producto.
   ...
   Pista: pensá en el "plan B" con ||: si no existe, arrancá de cero
   y sumale cantidad. Escribí vos la línea con la clave dinámica.
...
3) total() → suma de todas las cantidades del inventario.
   ...
   siempre. Es como sumar todo lo que hay en la caja registradora.
4) listar() → devuelve un string "nombre: cantidad, ..." con todos
   los productos. En el mismo ejemplo tenés la función que te da los
   pares [nombre, cantidad]: armá los pedazos con map y juntalos
   con join (el join lo viste en el módulo 06).
