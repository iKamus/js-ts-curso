...
- **Que es**: una funcion que se pasa como argumento para que se ejecute cuando algo termina. Es como dejarle el encargo al delivery: vos seguis con lo tuyo y, cuando llega, te avisa.
...
- **Buenas practicas**: si tenes mas de dos niveles de anidacion, pasa a promesas. Si la callback recibe un error, usalo como primer parametro (convencion de Node).
...
- **Cuando usarlo**: cuando tenes varias operaciones asincronas independientes y necesitas todos los resultados para continuar.
- **Sintaxis**:
  ```js
...
- **Cuando usarlo**: cuando necesitas el resultado de todas las promesas sin que una falla corte el resto. Ideal para operaciones donde podes tener exitos parciales.
- **Sintaxis**:
  ```js
...
- **Cuando usarlo**: cuando queres usar el resultado de la respuesta mas rapida, o implementar un timeout para una operacion que puede tardar demasiado.
- **Sintaxis**:
  ```js
...
- **Buenas practicas**: marca como `async` solo las funciones que usan `await`. Dentro de una funcion async, siempre envolve en `try/catch` las operaciones que pueden fallar.
...
- **Que es**: la forma nativa de hacer peticiones HTTP en Node 18+ y en todos los navegadores. Es como ir a buscar datos a otro lugar: le decis "traeme esto", esperas la respuesta y despues la revisas con calma.
...
- **Errores comunes**: `fetch` solo rechaza en errores de red (sin conexion, DNS). Si el servidor responde con 404 o 500, fetch no rechaza: tenes que chequear `res.ok` manualmente.
...
| Archivo | Tema |
|---|---|
| `ejemplos/01-callback.js` | callbacks con setTimeout y el orden asincrono |
| `ejemplos/02-promesas.js` | then/catch/finally, Promise.all, allSettled, race |
| `ejemplos/03-async-await.js` | async/await y try/catch |
| `ejemplos/04-fetch.js` | fetch a una API publica con manejo de errores (necesita internet) |
