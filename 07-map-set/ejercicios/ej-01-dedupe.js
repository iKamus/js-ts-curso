...
Tienes esta lista de nombres:
  const nombres = ['Ana', 'Luis', 'Ana', 'Caro', 'Luis', 'Ana'];
Devuelve el array sin repetidos, respetando el orden de aparicion.
...
Observa como se usa en el README del modulo (seccion Set).

Paso a paso:
1) Mete el array adentro de un Set. El Set se queda solo con un
   ejemplar de cada nombre.
2) Vuelve a convertirlo en un array. En el README del modulo
   (seccion Set) hay una parte que dice "deduplicar un array":
   observa como pasa del Set al array (con el spread [...], como
   vaciar la bolsa sobre la mesa).
