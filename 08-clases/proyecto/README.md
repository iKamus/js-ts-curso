...
1. Crear una clase base `Carta` con propiedad `palo` (privada) y metodo `描述ar()`.
...
6. Crear instancias y probar: una mano con cartas mixtas (numeros y figas), mostrar puntaje total y listado.
...
1. Crear la clase `Carta` con constructor que reciba `palo` y lo guarde como campo privado `#palo`. Crear getter `palo` y metodo `描述ar()`.
...
4. Crear `Mano` con campo privado `#cartas` (array vacio). Metodos: `agregar`, getter `total`, getter `cantidad`, metodo `描述ar()`.
5. Crear una mano, agregar 5 cartas (mezcla de numeros y figas), mostrar total y listado.
6. Usar `instanceof` dentro de `描述ar()` para indicar si es carta numero o figura.
